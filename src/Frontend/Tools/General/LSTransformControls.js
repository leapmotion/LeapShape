...
var LSTransformControls = function ( camera, domElement ) {

	if ( domElement === undefined ) {

		console.warn( 'THREE.LSTransformControls: The second parameter "domElement" is now mandatory.' );
		domElement = document;
	...
	var _gizmo = new LSTransformControlsGizmo();
	this.add( _gizmo );

	var _plane = new LSTransformControlsPlane();
	this.add( _plane );
	...
	this.updateMatrixWorld = function () {
		...
		if ( this.object !== undefined ) {
			...
			if ( this.object.parent === null ) {
				...
				console.error( 'LSTransformControls: The attached 3D object must be a part of the scene graph.' );
	...
	this.pointerHover = function ( pointer ) {
		...
		if (pointer.ray) {
			raycaster.set(pointer.ray.origin, pointer.ray.direction);
		} else {
		...
		if ( intersect ) {
			...
			if (pointer.ray) { pointer.hovering = true; }
	...
	this.pointerDown = function ( pointer ) {
		if ( this.object === undefined || this.dragging === true || (!pointer.active && pointer.button !== 0) ) return;
	...
	this.pointerMove = function ( pointer ) {
		...
		if ( object === undefined || axis === null || this.dragging === false || (!pointer.active && pointer.button !== - 1) ) return;
    ...
    /** This function is LeapShape specific; calling the requisite 
     * commands on interaction rays fed through the pipeline. 
     * @param {InteractionRay} interactionRay */
	this.update = function (interactionRay) {
		// This function is a deprecated no-op in the original script.
		if ( ! scope.enabled ) return;

		this.pointerHover(interactionRay);
		if (this.dragging) {
			interactionRay.hovering = interactionRay.hovering || scope.visible;
			this.pointerMove(interactionRay);
		}

        if (interactionRay.justActivated) {
            this.pointerDown(interactionRay);
        } else if (interactionRay.justDeactivated) {
            this.pointerUp(interactionRay);
        }// else {
        //    this.pointerHover(interactionRay);
        //}
	};
...
LSTransformControls.prototype = Object.assign( Object.create( Object3D.prototype ), {

	constructor: LSTransformControls,

	isLSTransformControls: true

} );


var LSTransformControlsGizmo = function () {

	'use strict';

	Object3D.call( this );

	this.type = 'LSTransformControlsGizmo';
...
LSTransformControlsGizmo.prototype = Object.assign( Object.create( Object3D.prototype ), {

	constructor: LSTransformControlsGizmo,

	isLSTransformControlsGizmo: true

} );


var LSTransformControlsPlane = function () {
	...
	this.type = 'LSTransformControlsPlane';
...
LSTransformControlsPlane.prototype = Object.assign( Object.create( Mesh.prototype ), {

	constructor: LSTransformControlsPlane,

	isLSTransformControlsPlane: true

} );

export { LSTransformControls, LSTransformControlsGizmo, LSTransformControlsPlane };
